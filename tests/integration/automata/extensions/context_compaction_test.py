...
def test_context_compaction_runtime_boundaries(tmp_path: Path) -> None:
    ...
    (pi_package / "index.js").write_text(
        ...
        "      ? {systemPrompt: 'shared-system', messages: [{role: 'user', content: [{type: 'text', text: 'shared'}]}]}"
        "      : undefined;"
        "    const invoke = async (systemPrompt, text) => {"
        "      const context = sharedContext ?? globalThis.__nativeContextOverride ?? {systemPrompt, messages: [{role: 'user', content: [{type: 'text', text}]}]};"
        ...
        "      if (preparation.messagesToSummarize.length > 0) await invoke('history-system', 'history prompt');"
